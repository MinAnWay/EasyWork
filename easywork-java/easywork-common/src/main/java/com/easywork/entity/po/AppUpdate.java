...
public class AppUpdate implements Serializable {
    private static final long serialVersionUID = -3377680112106668831L;
    /**
     * 自增ID
     */
    private Integer id;
    ...
    public String toString() {
        return "自增ID:" + (id == null ? "空" : id) + ",版本号:" + (version == null ? "空" : version) + ",更新描述:" + (updateDesc == null ? "空" : updateDesc) + ",更新类型 0:全更新 1:局部热更新:" + (updateType == null ? "空" : updateType) + ",创建时间:" + (createTime == null ? "空" : DateUtils.format(createTime, DateTimePatternEnum.YYYY_MM_DD_HH_MM_SS.getPattern())) + ",0:未发布 1:灰度发布 2:全网发布:" + (status == null ? "空" : status) + ",灰度设备ID:" + (grayscaleDevice == null ? "空" : grayscaleDevice);
    }
}